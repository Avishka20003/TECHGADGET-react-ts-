...
export const sendOrderConfirmationEmail = async (email: string, orderData: any) => {
  try {
    ...
    const mailOptions = {
      from: `"TechGadget Store" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: `Order Confirmed! ${orderData.invoiceNumber}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
          <h2 style="color: #1565c0; text-align: center;">Thank You for Your Order! 🚀</h2>
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="background: #1565c0; color: white;">
                <th style="padding: 10px; text-align: left;">Gadget Name</th>
          ...
          <h3 style="text-align: right; color: #1565c0; margin-top: 20px;">Total Amount: Rs. ${orderData.totalAmount}.00</h3>
          
          <hr style="border: 0; border-top: 1px solid #eee; margin-top: 30px;">
          <p style="font-size: 12px; color: #777; text-align: center;">TechGadget Store © 2026 | Upgrade Your Lifestyle!</p>
        </div>
      `,
    };
